...
from initializer import conv_agent_handler, redis_pubsub_manager
...
router = APIRouter(
    prefix="/conversational-agent",
    dependencies=[
        Depends(require_scopes_cached(APIScope.AGENT_CONVERSATION)),
    ],
)
...
async def chat(
    ...
    """
    Handle chat requests with the conversational agent.

    **Required Scopes:** `agent_conversation`
...
async def stream_events(
    ...
    **Required Scopes:** `agent_conversation`
    ...
    """
    await conv_agent_handler.initialize(conv_id, request)
